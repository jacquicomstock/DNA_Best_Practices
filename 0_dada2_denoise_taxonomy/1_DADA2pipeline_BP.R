library("dada2")
path <- "/home/jcomstock/M17/fastqs"
...
write(asv_fasta, "M17_ASVs.fasta")
taxa <- assignTaxonomy(seqtab.nochim, "silva_nr99_v138.1_wSpecies_train_set.fa.gz")
unname(head(taxa))
saveRDS(taxa, "taxa.rds")
write.table(cbind(t(seqtab.nochim) , taxa), "M17_seqtab-nochimtaxa.txt", sep="\t", row.names=TRUE, col.names=NA, quote=FALSE)
write.table(taxa,"M17_taxa.txt", sep="\t", row.names=TRUE, col.names=NA, quote=FALSE)
q()
